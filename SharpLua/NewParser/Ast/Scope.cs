...
namespace SharpLua.Ast
{
    public class Scope
    {
        ...
        public Variable CreateLocal(string name, bool includeParent = true)
        {
            Variable x = GetLocal(name, includeParent);
            if (x != null)
                return x;
        ...
        public Variable GetLocal(string name, bool includeParent = true)
        {
            foreach (Variable v in Locals)
                if (v.Name == name)
                    return v;
        ...
        /// top: true是时候，找child；false，找parent
        public List<Variable> GetLocals(bool includeSelf, bool includeParent, bool includeChild)
        {
            List<Variable> v = new List<Variable>();
            if(includeSelf && Locals!=null)
            {
                v.AddRange(Locals);
            }
            ...
            {
                v.AddRange( Parent.GetLocals(true,true,false));
            }

            if (includeChild && Children!=null)
            {
                foreach(Scope c in Children)
                {
                    v.AddRange(c.GetLocals(true, false, true));
                }
            }
        ...
        /// top: true是时候，找child；false，找parent
        public List<Variable> GetGlobals(bool includeSelf, bool includeParent, bool includeChild)
        {
            List<Variable> v = new List<Variable>();
            if (includeSelf && Globals != null)
            {
                v.AddRange(Locals);
            }
            ...
            if (includeParent && Parent != null)
            {
                v.AddRange(Parent.GetGlobals(true, true, false));
            }
            ...
            if (includeChild && Children != null)
            {
                foreach (Scope c in Children)
                {
                    v.AddRange(c.GetGlobals(true, false, true));
                }
            }
